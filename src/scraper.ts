...
import { type PropMapRules, extractFromArr, getUnit, txt } from './helpers'
...
export const createGOB = (session: string, token: string) => {
  const navJobs = async (
    ...
    if (process.env.DEBUG) {
      ...
      console.log('session', `_getonboard_session=${session};`)
    }
    ...
    return res.json() as Promise<{ jobs: Job[]; meta: JobSearchMeta }>
  }
  ...
  const getJob = async (slug: string): Promise<JobData> => {
    ...
    const salary = {
      type: _salary.find('.hide-on-small-mobile').text().match(/gross/i)
        ? 'gross'
        : 'net',
      min: +(_salary.find('[itemprop="minValue"]').attr('content') || 0),
      max: +(_salary.find('[itemprop="maxValue"]').attr('content') || 0),
    ...
    const propMapRules: PropMapRules = {
      ...
      repliesIn: [
        ...
        (match) => {
          ...
          if (match.match(/in \d*/))
            return [unit!, +match.match(/in (\d*)/)?.[1]!]
